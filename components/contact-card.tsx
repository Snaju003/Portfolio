...
export function ContactCard({ person }: { person: Person }) {
	...
	useEffect(() => {
		if (state?.message) {
			console.log("State has message, ok:", state.ok);
			if (state.ok) {
			...
			<motion.div
				...
				<Card>
					<CardHeader className="pb-3">
						...
						<p className="text-sm text-muted-foreground">
							{
								"Prefer email or a quick call? Reach me directly via the links below."
							}
						</p>
					</CardHeader>
					<CardContent className="space-y-3">
						<motion.div
							...
							<Link
								...
								className="inline-flex items-center gap-2 text-emerald-700 dark:text-emerald-400 hover:underline underline-offset-4">
